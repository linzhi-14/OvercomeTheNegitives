...
ServerEvents.tick(event => {
    ...
    //服务器废物清理提醒
    if(server_minute % 10 == 9) {
        if(server_second == 45 && server_tick % 20 == 0) {
            event.server.runCommandSilent('say 还有15秒扫地妈妈到达现场！')
        }
        else if(server_second == 55 && server_tick % 20 == 0) {
            event.server.runCommandSilent('say 还有5秒就要清除掉落物了喵')
        }
        else if(server_second == 57 && server_tick % 20 == 0) {
            event.server.runCommandSilent('say 3！')
        }
        else if(server_second == 58 && server_tick % 20 == 0) {
            event.server.runCommandSilent('say 2！')
        }
        else if(server_second == 59 && server_tick % 20 == 0) {
            event.server.runCommandSilent('say 1！')
        }
    }
    //服务器废物清理
    if ((server_minute % 10 == 0 && server_second == 0 && server_tick % 20 == 0)){
